...
| `test-api.js` | News fetchers (Blockchair + Hacker News parsing, clamping, filtering), the cross-source news merge (spam filter, title dedupe), on-chain address balance lookups (provider parsing, unit conversion, cache, stale-on-failure, junk guards) and per-coin page ticker snapshot (24h change math, TTL skip) |
...
| `test-portfolio.js` | Portfolio: holdings persistence incl. total-paid and watched-address fields (whitelist, dedupe, number coercion, per-chain address validation, corrupt-JSON fallback), the `sanitizePortfolio` import validator, chart period setting ("hour" excluded), total-value series builder (amount × price sums, end-alignment, shortest-history trim), the tax-report CSV builder (P/L math, derived avg cost, totals, escaping) and the history cache (TTL, stale-on-failure fallback) |
