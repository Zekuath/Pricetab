...
| `test-portfolio.js` | Portfolio: holdings persistence incl. avg-cost field (whitelist, dedupe, number coercion, corrupt-JSON fallback), the `sanitizePortfolio` import validator, chart period setting ("hour" excluded), total-value series builder (amount × price sums, end-alignment, shortest-history trim), the tax-report CSV builder (P/L math, totals, escaping) and the history cache (TTL, stale-on-failure fallback) |
