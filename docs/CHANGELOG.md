...
- **Portfolio v2** - optional "total paid" input per holding ("I bought this much for this much") unlocks real unrealized P/L per row and as a headline stat; allocation share per holding; JSON backup/restore (validated against the coin whitelist on import); "Tax report" CSV export with cost basis, derived avg cost and unrealized P/L per coin (informational only, not tax advice)
- **Address watching** - optionally watch your own BTC/ETH/LTC/DOGE address: the holding's amount stays synced to the address's public on-chain balance (read-only lookup via mempool.space / Blockchair, 10-minute cache; the address is stored locally and sent only to the balance provider)
