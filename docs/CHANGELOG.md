...
- **Coin badges** - the quick jumper and the alert list show a small monogram badge with a colour derived from the coin's symbol, making long lists easier to scan. No brand logo files and no external requests — the extension still makes zero third-party asset calls
...
- **Since your last visit** - a quiet line under the price shows how the coin moved since you last looked ("Since your last visit (3h ago) +1.24% · $512"), switchable from Settings → Appearance. The baseline only refreshes after 20 minutes, so opening a burst of tabs keeps comparing against the same earlier visit instead of resetting to "just now"
- **Chart crosshair with OHLC + volume** - hovering the chart reads out that point's open, high, low, close and volume (from Coinbase candles, fetched only once you actually hover — tabs you just glance at cost nothing extra). Ranges or currencies without candle data fall back to the price and date (clock times on 1H/1D, plain dates on longer ranges); the readout follows the nearest data point, flips sides at the edges and stays inside the chart. Built to stay cheap: no React re-render per pointer move, a binary search over pre-scaled points, and DOM writes batched into one animation frame
