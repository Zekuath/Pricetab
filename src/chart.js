...
class LineBase extends PureComponent {
  constructor(...args) {
    ...
    _defineProperty(this, "updateCalls", () => {
      ...
      calls
        ...
        .forEach((c) => {
          ...
          /* One click on a locked box, and it is asking whether you meant it.
           * Drawn in neutral ink for the same reason the release ghost is:
           * green and red on this chart already mean the *answer*, and a call
           * being taken back has none. */
          const asking = !c.result && c.id === this.unlockId;
          const colour = asking
            ? color.text
            : hit
              ? color.chartLineGreen
           ...
           * The one being asked about gets a looser dash than a live call: the
           * grip is already coming off. */
          if (c.result) box.removeAttribute("stroke-dasharray");
          else box.setAttribute("stroke-dasharray", asking ? "2 5" : "3 3");
     ...
     */
    /* Where one call sits on screen right now.
     *
     * The same four lines the celebration already does for the box that came
     * true — lifted here so the release animation and the burst cannot end up
     * with two ideas of where a call is drawn. Null when the chart has no
     * scales yet, or when the call has panned off the window: there is nothing
     * to animate at coordinates that are not on the chart. */
    _defineProperty(this, "callBox", (c) => {
      if (!c || !this.timeToX || !this.priceToY) return null;
      const x1 = this.timeToX(new Date(c.target - c.span));
      const x2 = this.timeToX(new Date(c.target));
      const y1 = this.priceToY(c.hi);
      const y2 = this.priceToY(c.lo);
    ...
    /* The walls letting go.
     *
     * The inverse of the latch: locking draws a square growing *out to* the
     * walls and the walls flashing as it lands, so releasing is the walls
     * opening and the box dissolving outward. Same duration family, same
     * easing.
     *
     * **Neutral ink, not green or red.** On this chart those two already mean
     * the *answer* — green called it, red missed — and a withdrawn call has no
     * answer. It is the same reason the portfolio's buy and sell markers are
     * neutral: the colour channel is already spoken for.
     *
     * Drawn in the burst layer rather than `_extraLayer`, which `updateCalls`
     * clears at the top of every redraw — and the redraw that removes the call
     * lands immediately after this. */
    _defineProperty(this, "release", (box) => {
      const layer = this.burstRef.current;
      if (!layer || !box) return;
      while (layer.firstChild) layer.removeChild(layer.firstChild);
      const { color } = this.props.theme;
      ...
      ghost.setAttribute("x", box.x);
      ghost.setAttribute("y", box.y);
      ghost.setAttribute("width", box.w);
      ghost.setAttribute("height", box.h);
      ghost.setAttribute("fill", "none");
      ghost.setAttribute("stroke", color.text);
      ghost.setAttribute("stroke-width", "1.4");
      ghost.setAttribute("stroke-dasharray", "3 3");
      ghost.setAttribute("rx", "2");
      layer.appendChild(ghost);
      const grow = Math.min(14, Math.max(6, Math.min(box.w, box.h) * 0.3));
      select(ghost)
    ...
    _defineProperty(this, "handleChartClick", (e) => {
      ...
      if (!cell) {
        /* A click anywhere else abandons the gesture rather than leaving a
         * half-made call — or a half-asked question — sitting on the chart. */
        if (this.draftAt || this.unlockId) {
          this.draftAt = null;
          this.unlockId = null;
          this.updateCalls();
        }
      ...
      /* A locked call can be taken back, and it takes two clicks like
       * everything else on this board.
       *
       * It could not, and the comment here said a record you cannot edit is
       * the whole point. That was true of *this surface* and false of the
       * app: the calls panel has always carried a `×` that withdraws an open
       * call with no confirmation at all. So the rule was not "a call is
       * final", it was "a call is final unless you know where the panel is" —
       * which is a worse rule, because the person most likely to have locked
       * the wrong square by accident is the one looking at the chart.
       *
       * **A settled call really is final.** Once it has an answer there is
       * nothing to take back; withdrawing it then would be editing the record
       * rather than correcting a slip, and that is the line this keeps.
       *
       * Held by **id, not by pointer position**. `draftAt` holds a position
       * because the square it names is re-derived from a price domain that
       * moves between the two clicks — the bug that made the second click
       * draft instead of lock. A call is a stored record with a stable id:
       * there is nothing to re-derive, so there is nothing to drift. */
      const taken = this.callOccupying(cell);
      if (taken) {
        /* One gesture at a time: reaching for a locked box abandons a draft.
         * `unlockId` is **not** cleared here — the whole point of the branch
         * below is to compare it with the box just clicked. */
        this.draftAt = null;
        if (taken.result) {
          this.unlockId = null;
          this.updateCalls();
        ...
        if (this.unlockId === taken.id && this.props.onWithdrawCall) {
          this.unlockId = null;
          // The walls let go before the box is gone — drawn in the burst
          // layer, which `updateCalls` does not clear, so it outlives the
          // redraw that removes the call it belonged to
          this.release(this.callBox(taken));
          this.props.onWithdrawCall(taken.id);
          this.updateCalls();
          this.drawGridCell();
          return;
        }
        this.unlockId = taken.id;
        this.updateCalls();
        this.drawGridCell();
        return;
      }
    ...
    /* The board's geometry — twenty-five members, 782 lines — lives in
     * `chart-board.js`. The seam is the one this file already drew for
     * itself: everything there answers *how big is a square and where does
     * the window sit*, and `updateGrid` below answers *put it on screen*.
     *
     * There was no block of pure helpers to lift instead: measured before the
     * cut, 95 members here and three of them touch no `this`. Same idiom as
